...
export default {async fetch(request,env){
 try{
  ...
  if(url.pathname==='/__login'){
   ...
   const form=new URLSearchParams(text),password=form.get('password')||'',returnTo=safeReturn(form.get('returnTo'),url.origin);
   if(!await passwordMatches(password,env.STAGING_PASSWORD_HASH)){const result=loginPage('That password was not accepted.',returnTo);return secure(new Response(result.body,{status:401,headers:result.headers}))}
   return secure(new Response(null,{status:303,headers:{Location:returnTo, 'Set-Cookie':`${cookieName}=${await session(env.SESSION_SECRET)}; Path=/; HttpOnly; Secure; SameSite=Strict; Max-Age=${ttl}`}}));
  }
