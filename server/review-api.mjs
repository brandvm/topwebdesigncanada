...
async function route(request,env,url){
 ...
 if(method==='POST'&&parts[0]==='threads'&&parts.length===1){
  ...
  if(old){if(old.page!==page||old.scope!==scope||old.visitor_id!==visitor||old.body!==body||old.name!==name||old.anchor!==anchor||old.x!==x||old.y!==y||old.width!==w||old.height!==h||old.commit_id!==commit)fail(409,'Request already used.');return{id:old.id}}
  ...
  const now=Date.now();await db.batch([
   db.prepare('INSERT INTO review_threads (request_id,page,scope,commit_id,viewport_width,viewport_height,anchor,anchor_label,x,y,width,height,selection_type,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(request_id) DO NOTHING').bind(requestId,page,scope,commit,vw,vh,anchor,label,x,y,w,h,type,now),
