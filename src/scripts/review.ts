...
function positionPopover(){
 ...
 panel.hidden=false;const layout=attachedCard({x:pinX,y:Math.min(pinY,usableHeight-24)},panel.getBoundingClientRect(),{width:innerWidth,height:usableHeight});if(!layout){panel.hidden=true;return;}
...
addEventListener('scroll',()=>{if(gesture)gesture.end={x:gesture.clientX+scrollX,y:gesture.clientY+scrollY};schedulePins();},{passive:true});addEventListener('resize',schedulePins);visualViewport?.addEventListener('resize',schedulePins);visualViewport?.addEventListener('scroll',schedulePins);new ResizeObserver(schedulePins).observe(document.getElementById('site-mode')!);new ResizeObserver(schedulePins).observe(document.body);document.fonts.ready.then(schedulePins);document.querySelectorAll('img').forEach(img=>img.addEventListener('load',schedulePins,{once:true}));
